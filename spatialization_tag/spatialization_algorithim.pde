/*
Name: spatialization_algorithim.pde
* 
Authorship:
...
This code intakes the compressed audio signal through UARTs from the receiver_tag code, decodes it using the ADPCM algorithm, 
  and then applies spatialization effects based on the angle data received from the u-locate module. 
It also takes in the angle of arrival (AoA) data from the u-locate module through a separate UART port,
  which is used to calculate the necessary ITD and ILD for spatialization. 
The spatialization is achieved by calculating interaural time delay (ITD) and interaural level differences (ILD) 
  based on the angle of arrival, and then applying these to the audio signal before outputting it through the audio shield. 
