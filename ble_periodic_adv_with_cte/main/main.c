...
static uint8_t eddystone_uid[] = {
    ...
    /* Namespace (10 bytes) */
    0x4E,0x49,0x4E,0x41,0x2D,
    0x42,0x34,0x54,0x41,0x47,
...
static void start_periodic_adv_cte(void)
{
    ...
    data = os_msys_get_pkthdr(sizeof(eddystone_uid), 0);
    assert(data);

    rc = os_mbuf_append(data, eddystone_uid, sizeof(eddystone_uid));
    assert(rc == 0);
